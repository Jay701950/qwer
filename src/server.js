...
export function rewriteHtml(html, baseUrl) {
  let output = html;
  output = output.replace(/(\b(?:href|src|action|poster|cite|background)\s*=\s*["'])([^"']+)(["'])/gi,
...
async function proxyRequest(req, res) {
  ...
  try {
    ...
    res.status(upstream.status).set("x-proxy-upstream", target.origin).set("cache-control", "no-store");
    if (/^text\/html(?:;|$)/i.test(contentType)) return res.send(rewriteHtml(body.toString("utf8"), target));
    if (/^text\/css(?:;|$)/i.test(contentType)) return res.send(body.toString("utf8").replace(/url\(\s*(["']?)([^)'"\s]+)\1\s*\)/gi, (_m, q, value) => `url(${q}${rewriteReference(value, target)}${q})`));
